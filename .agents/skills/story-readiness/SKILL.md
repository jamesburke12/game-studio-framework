...
**Scope:** `$ARGUMENTS[0]` (blank = ask user via ask the user (see docs/vendor-map.md))
...
If no argument is given, use `ask the user (see docs/vendor-map.md)`:
...
- **GAPS [list]** → surface the specific gaps to the user via `ask the user (see docs/vendor-map.md)`:
