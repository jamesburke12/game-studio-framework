...
- `docs/architecture/control-manifest.md` (universal rules + Manifest Version date from the header) and `docs/architecture/control-manifest/<system>.md` — the slice for this epic's system only
- `docs/architecture/tr/<system>.yaml` — the slice for this system; load all its TR-IDs and no other slice
...
- **TR-ID**: look up in `tr/<system>.yaml` (the system's slice). Use the stable ID. If no match, use `TR-[system]-???` and warn.
...
*(Requirement text lives in `docs/architecture/tr/<system>.yaml` — read that slice fresh at review time)*
...
Use `ask the user (see docs/vendor-map.md)` to close with context-aware next steps:
