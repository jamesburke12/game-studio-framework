...
Use `ask the user (see docs/vendor-map.md)` for write permission:
- Prompt: "May I write this review to `design/gdd/gdd-cross-review-[date].md`?"
- Options: `[A] Yes — write the report` / `[B] No — skip`

If any GDDs are flagged for revision, use a second `ask the user (see docs/vendor-map.md)`:
...
After all file writes are complete, use `ask the user (see docs/vendor-map.md)` for a closing widget.
