...
3. If multiple in-progress stories are found, use `ask the user (see docs/vendor-map.md)`:
...
Also read:
- `docs/architecture/tr/<system>.yaml` — the slice for the story's system; look up each TR-ID in the story.
...
### Manual verification with confirmation (use `ask the user (see docs/vendor-map.md)`)
...
Batch up to 4 manual verification questions into a single `ask the user (see docs/vendor-map.md)` call:
...
1. Ask: is there a test — unit, integration, or confirmed manual playtest — that
   ...
   - **Unit test**: check `assets/Tests/unit/` for a test file or function name that
     matches the criterion's subject (use `Glob` and `Grep`)
   - **Integration test**: check `assets/Tests/integration/` similarly
   - **Manual confirmation**: if the criterion was verified via `ask the user (see docs/vendor-map.md)`
...
1. **GDD rules check**: Using the current requirement text from `tr/<system>.yaml`
...
3. **ADR constraints check**: Read the referenced ADR's Decision section. Check
   for forbidden patterns from `docs/architecture/control-manifest.md` and `docs/architecture/control-manifest/<system>.md` (if it
...
- `lean` → use `ask the user (see docs/vendor-map.md)` before proceeding:
...
Present the verdict to the user. If CONCERNS, surface them via `ask the user (see docs/vendor-map.md)`:
...
Use `ask the user (see docs/vendor-map.md)` before writing anything:
...
- Use `ask the user (see docs/vendor-map.md)` for the code review prompt and for batching manual
