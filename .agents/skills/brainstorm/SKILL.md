...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Use `ask the user (see docs/vendor-map.md)`** at key decision points throughout brainstorming:
   ...
   `ask the user (see docs/vendor-map.md)` to capture the decision with concise labels.
...
- What 3 games have you spent the most time with? What kept you coming back?
  *(Ask this as plain text — the user must be able to type specific game names freely.
  Do NOT put this in an ask the user (see docs/vendor-map.md) with preset options.)*
- Are there genres you love? Genres you avoid? Why?
- Do you prefer games that challenge you, relax you, tell you stories,
  or let you express yourself? *(Use `ask the user (see docs/vendor-map.md)` for this — constrained choice.)*

**Practical constraints** (shape the sandbox before brainstorming).
Bundle these into a single multi-tab `ask the user (see docs/vendor-map.md)` with these exact tab labels:
...
Present all three. Then use `ask the user (see docs/vendor-map.md)` to capture the selection.

**CRITICAL**: This MUST be a plain list call — no tabs, no form fields. Use exactly this structure:

```
ask the user (see docs/vendor-map.md)(
...
Ask these as `ask the user (see docs/vendor-map.md)` calls — derive the options from the chosen concept, don't hardcode them:
...
**Pillar confirmation**: After presenting the full pillar set, use `ask the user (see docs/vendor-map.md)`:
...
If the user selects B, C, or D, make the revision, then use `ask the user (see docs/vendor-map.md)` again:
...
Collect both verdicts, then present them together using a two-tab `ask the user (see docs/vendor-map.md)`:
...
- **Target platform**: Use `ask the user (see docs/vendor-map.md)` — "What platforms are you targeting for this game?"
...
- **Engine experience**: Use `ask the user (see docs/vendor-map.md)` — "Do you already have an engine you work in?"
...
5. Use `ask the user (see docs/vendor-map.md)` for write approval:
...
If [B]: ask which section to revise using `ask the user (see docs/vendor-map.md)` with options: `Elevator Pitch` / `Core Fantasy & Unique Hook` / `Pillars` / `Core Loop` / `MVP Definition` / `Scope Tiers` / `Risks` / `Something else — I'll describe`

After revising, show the updated section as a diff or clear before/after, then use `ask the user (see docs/vendor-map.md)` — "Ready to write the updated concept document?"
