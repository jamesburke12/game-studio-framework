...
- Use `ask the user (see docs/vendor-map.md)` to ask:
...
Then use `ask the user (see docs/vendor-map.md)` to capture feedback:
...
Use `ask the user (see docs/vendor-map.md)` to ask: "Does this dependency ordering look right? Any
...
Use `ask the user (see docs/vendor-map.md)` to ask: "Do these priority assignments match your vision?
...
Use `ask the user (see docs/vendor-map.md)` for: "Start designing [system-name] now, pick a different
system, or stop here?"
...
After `/design-system` completes, use `ask the user (see docs/vendor-map.md)`:
...
After the systems index is created (or after designing some systems), present next actions using `ask the user (see docs/vendor-map.md)`:
