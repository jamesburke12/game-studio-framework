...
a **confirm/adjust** prompt using `ask the user (see docs/vendor-map.md)` — not open-ended questions.
...
decisions remain, present each one as a separate `ask the user (see docs/vendor-map.md)` with the proposed
...
5. **Write approval** — Use `ask the user (see docs/vendor-map.md)`:
...
Ask using `ask the user (see docs/vendor-map.md)`:
...
After the ADR is written (and registry optionally updated), close with `ask the user (see docs/vendor-map.md)`.
