...
**Argument:** `$ARGUMENTS` (blank = ask user via ask the user (see docs/vendor-map.md))
...
- **No argument** — use `ask the user (see docs/vendor-map.md)`:
...
- `docs/architecture/control-manifest.md` and `docs/architecture/control-manifest/<system>.md` (system from the story's TR ids) — scan for forbidden patterns that
...
then ask two questions together using `ask the user (see docs/vendor-map.md)`:
...
- Use `ask the user (see docs/vendor-map.md)` for scope selection when no argument is provided.
