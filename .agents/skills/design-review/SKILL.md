...
Use `ask the user (see docs/vendor-map.md)` for ALL closing interactions. Never plain text.
...
Work through all blocking items, asking for design decisions only where you cannot resolve the issue from the GDD and existing docs alone. Group all design-decision questions into a single multi-tab `ask the user (see docs/vendor-map.md)` before making any edits — do not interrupt mid-revision for each blocker individually.

After all revisions are complete, show a summary table (blocker → fix applied) and use `ask the user (see docs/vendor-map.md)` for a **post-revision closing widget**:
...
When the verdict is APPROVED, use a single `ask the user (see docs/vendor-map.md)` with `multiSelect: true` to batch the two tracking updates:
...
Use a second `ask the user (see docs/vendor-map.md)`:
...
Use a third `ask the user (see docs/vendor-map.md)`:
...
Once the systems-index and review-log widgets are answered, check project state and show one final `ask the user (see docs/vendor-map.md)`:
