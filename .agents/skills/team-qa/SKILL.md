...
Present the qa-lead's full strategy to the user, then use `ask the user (see docs/vendor-map.md)`:
...
Use `ask the user (see docs/vendor-map.md)` per story group (batched 3-4 at a time):
...
Batch stories into groups of 3-4 and use `ask the user (see docs/vendor-map.md)` for each:
...
After each FAIL result: use `ask the user (see docs/vendor-map.md)` to collect the failure description, then spawn `qa-tester` via Task to write a formal bug report in `production/qa/bugs/`.
