...
**Decision Points:** At each step transition, use `ask the user (see docs/vendor-map.md)` to present
...
Use dispatch a subagent (see docs/vendor-map.md) to spawn each team member as a subagent:
...
**Gate**: Use `ask the user (see docs/vendor-map.md)` to present all three Step 1 outputs (narrative brief, lore foundation, visual direction targets) and confirm before proceeding to Step 2.
...
Use `ask the user (see docs/vendor-map.md)` with options:
...
**Gate**: Use `ask the user (see docs/vendor-map.md)` to present Step 2 layout (including any unresolved adjacent area dependencies) and confirm before proceeding to Step 3.
...
**Gate**: Use `ask the user (see docs/vendor-map.md)` to present Step 3 outputs and confirm before proceeding to Step 4.
...
**Gate**: Use `ask the user (see docs/vendor-map.md)` to present both Step 4 results. If the accessibility-specialist returned any BLOCKING concerns, highlight them prominently and offer:
