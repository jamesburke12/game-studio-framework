...
2. **Check for existing test infrastructure**:
   ...
   - Glob `assets/Tests/unit/` and `assets/Tests/integration/` — do subdirectories exist?
   - Glob `.github/workflows/` — does a CI workflow file exist?
   - Glob `assets/Tests/EditMode/` and `assets/Tests/PlayMode/` for the Unity
     test assemblies. (This project is Unity only — TASK #0564.)
...
Create `assets/Tests/EditMode/` placeholder file `tests/EditMode/README.md`:
```markdown
# Edit Mode Tests
...
> **Removed 2026-08-28 (TASK #0564).** A Godot 4 CI-workflow template stood
> here. It interpolated a version from a Godot engine-reference file that never existed
> — **the same non-existent path that misled a `unity-specialist` today** via a
> stale line in `docs/CLAUDE.md`. Two files pointing at one phantom path is a
> pattern, not a coincidence. This project is Unity; see the Unity section below.
