...
**If no argument is provided**, do not fail — ask instead. Use `ask the user (see docs/vendor-map.md)`:
...
2. **Questions**: Ask what is needed to draft this section. Use `ask the user (see docs/vendor-map.md)`
...
3. **Options**: Where design choices exist, present 2-4 approaches with pros/cons.
   Explain reasoning in conversation, then use `ask the user (see docs/vendor-map.md)` to capture the decision.
4. **Decision**: User picks an approach or provides custom direction.
5. **Draft**: Write the section content in conversation for review. Flag provisional
   assumptions explicitly.
6. **Approval**: Use `ask the user (see docs/vendor-map.md)`:
...
7. **Write**: Use `ask the user (see docs/vendor-map.md)`: "May I write the [section name] section to `[filepath]`?"
...
- Use `ask the user (see docs/vendor-map.md)` to capture the choice:
...
- Use `ask the user (see docs/vendor-map.md)`: "Want an ASCII wireframe as part of this spec?"
...
Use `ask the user (see docs/vendor-map.md)` to confirm:
...
Use `ask the user (see docs/vendor-map.md)` to step through items in groups of 3-4, not all at once.
...
Work through patterns in groups. Use `ask the user (see docs/vendor-map.md)`:
...
Then use `ask the user (see docs/vendor-map.md)`:
...
When delegating to another agent via dispatch a subagent (see docs/vendor-map.md):
...
2. **ask the user (see docs/vendor-map.md)** at every decision point (Explain -> Capture pattern):
