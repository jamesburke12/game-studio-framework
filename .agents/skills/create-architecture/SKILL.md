...
Display a one-paragraph summary of what the document will contain (layers, modules, data flows, ADR gaps). Then use `ask the user (see docs/vendor-map.md)`:
...
Use `ask the user (see docs/vendor-map.md)` — "Technical Director and Lead Programmer have reviewed the architecture. How would you like to proceed?"
...
Show the proposed Document Status block inline, then use `ask the user (see docs/vendor-map.md)`:
...
> - [ ] Run `/test-setup` — scaffolds `assets/Tests/unit/`, `assets/Tests/integration/`, CI workflow, and an example test file
...
5. **Use `ask the user (see docs/vendor-map.md)` for write approvals** — plain text "May I?" is not
...
- Run `/test-setup` to scaffold `assets/Tests/unit/`, `assets/Tests/integration/`, CI workflow, and an example test (required for gate-check)
