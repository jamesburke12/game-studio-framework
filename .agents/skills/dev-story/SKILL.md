...
|------|------|------------|
| TR registry slice | `docs/architecture/tr/<system>.yaml` (system from the story's TR ids) | **STOP** — "TR registry slice not found at `docs/architecture/tr/<system>.yaml`. Run `/architecture-review` to bootstrap the registry from your GDDs and ADRs." |
...
| Control manifest | `docs/architecture/control-manifest.md` + `docs/architecture/control-manifest/<system>.md` | **WARN and continue** — "Control manifest not found — layer rules cannot be checked. Run `/create-control-manifest`." |
...
### The TR registry
Read the slice for the story's system: `docs/architecture/tr/<system>.yaml` and `docs/architecture/control-manifest/<system>.md` (system from the story's TR ids). Load no other slice. Look up the story's TR-ID.
...
Read `docs/architecture/control-manifest.md` (universal rules) and the slice for the story's system: `docs/architecture/tr/<system>.yaml` and `docs/architecture/control-manifest/<system>.md` (system from the story's TR ids). Extract the rules for this story's layer:
- Required patterns
- Forbidden patterns
- Performance guardrails

Check: does the story's embedded Manifest Version match the current manifest header date?
If they differ, use `ask the user (see docs/vendor-map.md)` before proceeding:
...
3. If any dependency has Status other than `Complete` or `Done`:
   - Use `ask the user (see docs/vendor-map.md)`:
...
|--------|----------------------------|
<!-- The Godot 4 row was removed 2026-08-28 (TASK #0564): those five agents were
     deleted from .claude/agents/, so the row routed to nothing. This project is
     Unity. -->
...
2. **GDD requirement**: look up TR-ID `[TR-XXX-NNN]` in `docs/architecture/tr/<system>.yaml` (the slice for the story's system) — use the `requirement` field as source of truth
3. **ADR**: `docs/architecture/[adr-file].md` — read the **Decision** and **Implementation Guidelines** sections only
4. **Control manifest**: `docs/architecture/control-manifest.md` plus `docs/architecture/control-manifest/<system>.md` — the system slice, and the universal rules only
...
3. **Offer options** via ask the user (see docs/vendor-map.md) with choices:
