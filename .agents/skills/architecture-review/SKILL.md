...
If [B]: display the complete proposed systems-index section, then re-ask with `ask the user (see docs/vendor-map.md)`.
...
Use `ask the user (see docs/vendor-map.md)` for the write approval:
...
For `rtm` mode, use `ask the user (see docs/vendor-map.md)`:
...
2. **Pre-gate checklist**: Check whether these exist via Glob and mark each ✅ or ❌:
   - `assets/Tests/unit/` and `assets/Tests/integration/` directories — if ❌: run `/test-setup`
...
Then close with `ask the user (see docs/vendor-map.md)` tailored to the pre-gate checklist state:
...
3. **Offer options** via ask the user (see docs/vendor-map.md) with three choices:
