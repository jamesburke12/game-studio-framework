...
- If it exists: read it, find the first entity or screen with status "Needed" but no spec file yet, and use `ask the user (see docs/vendor-map.md)`:
...
Present the full proposed inventory to the user in conversation. Then use `ask the user (see docs/vendor-map.md)`:
...
- **system**: Read `design/gdd/[target-name].md`. Extract the **Visual/Audio Requirements** section. If it doesn't exist or reads `[To be designed]`:
  ...
  Use `ask the user (see docs/vendor-map.md)`: `[A] Describe needs manually` / `[B] Stop — complete the GDD first`
...
- **character** or **entity**: Read `design/narrative/characters/[target-name].md` or search `design/narrative/` and `design/assets/entity-inventory.md` for a matching entry. Extract visual description, role, and any specified distinguishing features.
  - **If no source doc exists**: do not fail. Instead, use `ask the user (see docs/vendor-map.md)`:
...
Present the full identified list to the user. Use `ask the user (see docs/vendor-map.md)`:
...
After presenting all specs, use `ask the user (see docs/vendor-map.md)`:
