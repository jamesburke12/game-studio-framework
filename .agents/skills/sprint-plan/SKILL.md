...
- If the file doesn't exist and this is a `new` sprint: use `ask the user (see docs/vendor-map.md)`:
...
3. Ask the user what to change: stories to add, remove, reprioritize, or re-estimate. Use `ask the user (see docs/vendor-map.md)` to gather changes.
...
If CONCERNS, use `ask the user (see docs/vendor-map.md)`:
...
After writing, in the same turn (no further approval — writing the plan is the approval):

1. **Rotate the closed sprint.** Move the previous sprint's `sprint_<N-1>_stories:` block out of `production/sprint-status.yaml` and append it to `production/sprints/_closed/sprint-status-01-<N-1>.yaml` (rename the file to the new upper bound). The live yaml holds the active sprint only.
2. **One GitHub issue per story.** For every story row: `gh issue create --title "<story title>" --milestone "Sprint <N>" --label "story:<N>-<nn>" --label "role:<owner>" --label "status:ready" --label "tier:worker" --label "vendor:any" --label "size:S" --body-file <tmp>` where the body is the story's acceptance text plus `Story: production/sprints/sprint-<N>.md#<N>-<nn>`. Owner comes from the story's Owner column; gated paths (design/**, docs/architecture/**, docs/legal/**) make it `tier:lead`. Create the milestone first if missing (`gh api repos/:owner/:repo/milestones -f title="Sprint <N>"`).
3. **Parity check.** `gh issue list --milestone "Sprint <N>" --state all --limit 200 --json number --jq length` must equal the story count; fix any gap before starting work and report it.
