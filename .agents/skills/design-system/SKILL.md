...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and use `ask the user (see docs/vendor-map.md)`:
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   `ask the user (see docs/vendor-map.md)` for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use `ask the user (see docs/vendor-map.md)` to capture the decision.
...
6. **Approval**: Immediately after the draft — in the SAME response — use
   `ask the user (see docs/vendor-map.md)`. **NEVER use plain text. NEVER skip this step.**
...
**Framing questions (ask BEFORE drafting)**: Use `ask the user (see docs/vendor-map.md)` with a multi-tab widget:
...
**Framing question (ask BEFORE drafting)**: Use `ask the user (see docs/vendor-map.md)`:
...
- Surface any disagreements between agents to the user via `ask the user (see docs/vendor-map.md)`
...
- Present the specialists' proposals to the user for review via `ask the user (see docs/vendor-map.md)`
...
**When delegating via dispatch a subagent (see docs/vendor-map.md)**:
...
- The main session presents the agent's output to the user via `ask the user (see docs/vendor-map.md)`
- The user decides; the main session writes to file
